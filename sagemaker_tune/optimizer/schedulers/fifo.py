...
from sagemaker_tune.optimizer.schedulers.searchers.searcher import BaseSearcher
from sagemaker_tune.optimizer.schedulers.searchers.searcher_factory import \
    searcher_factory
from sagemaker_tune.optimizer.schedulers.searchers.utils.default_arguments \
    import check_and_merge_defaults, Categorical, String, Boolean, \
    assert_no_invalid_options, Integer
from sagemaker_tune.optimizer.schedulers.random_seeds import \
    RandomSeedGenerator
from sagemaker_tune.optimizer.scheduler import TrialScheduler, \
    SchedulerDecision, TrialSuggestion
from sagemaker_tune.backend.time_keeper import TimeKeeper, RealTimeKeeper
...
class FIFOScheduler(ResourceLevelsScheduler):
    ...
    def set_time_keeper(self, time_keeper: TimeKeeper):
        ...
        not yet started.
        """
        assert self.time_keeper is None, \
            "Time keeper has already been assigned"
        assert isinstance(time_keeper, TimeKeeper), \
    ...
    def _elapsed_time(self):
        ...
        """
        assert self.time_keeper is not None, \
